load("C:/Users/19198/OneDrive/Desktop/April26Omicsdata.RData")


################################### clean X ##########################################
X = X[order(X$subject_id), ]

m = as.numeric(as.factor(X$subject_id))
M= max(m)
X = X[,-c(5, 7)]

data_XY = cbind(Y = Y[,1], X)
model.matrix(lm(Y~., data = data_XY))

X = model.matrix(lm(Y~., data = data_XY))
rm(data_XY)
...
ql.matrix = sapply(1:n, function(x) quantile(Y, ql))
dl.new = c()
...
ls = list()
## hyper-parameter
vr = mean(hat.ri); Lr = 5; a.phi.r = 1; a.omega = b.omega = 5; #ur2 = var(hat.ri)
ur2 = 1
a.xi = vr
sigma2.xi = 1

vr.theta = mean(hat.thetaj); Lr.theta = 3; a.phi.theta = 1; a.omega.theta = b.omega.theta = 5; #ur2.theta = var(hat.thetaj)
ur2.theta = 1
a.xi.theta = vr.theta
sigma2.xi.theta = 1
...
index.zero.true = which(Y==0, arr.ind = T)
...
for(i in 1:niter){
  ...
  for(isample in 1:n){
    for(j in 1:J){
      if( Y [isample, j] >0){
        ...
        prob.mean = ri[isample] + thetaj[j] + sum(Lambda[j,] * eta[isample,]) + 
          sum(X[isample,] * beta.p[j,])
        prob.1 = (1-prob.0) * pnorm(0, mean = prob.mean, sd = sqrt(sigma2+vs2))
  ...
  for(j in 1:J){
    kappa.matrix[j, ] = mvtnorm::rmvnorm(1, mean =  
                                           pos.var.kappa %*% (solve(Sigma_kappa) %*% mu_kappa + t(tilde.X) %*% Z.matrix[,j,drop=F] ), 
                                         sigma = pos.var.kappa )
  }
  ...
  ### update vs2
  vs2 = 1/rgamma(1, n*J/2 + a.vs2, b.vs2 + sum(sij.matrix^2)/2)
...
save.image("C:/Users/19198/OneDrive/Desktop/April27result.RData")
