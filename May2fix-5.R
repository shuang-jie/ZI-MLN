rm(list = ls())
load("D:/UCSC 2021 Spring/April28/April26Omicsdata.RData")

################################### clean X ####################################
re_label = order(X$subject_id)
X = X[re_label, ]
...
mu_kappa = matrix(rep(qnorm(nrow(which(Y[1:20,]==0, arr.ind = T))/(n*J)), p2+1), ncol = 1); Sigma_kappa = diag(3, p2+1)
...
save.image("D:/UCSC 2021 Spring/April30/May2fix-5.RData")
