...
ZI_MLN_without <- function(Y, M = M, m = m, K=10, niter = 20000, seed = 3, a.phi = 1/2, a.tau = 1, b.tau = 1/50, a.sig = 3, b.sig = 3,
  ...
  eps.j = matrix(runif(n*J), nrow = n, ncol =J)
  ...
  RSS = y.star - matrix(ri, n, J) - matrix(thetaj, n, J, byrow = T) - sij - tcrossprod(eta, Lambda)
  logY = log(Y)
  logY.1 = log(Y+1)
  ...
  for(i in 1:niter){
    ...
    Z[index.delta.1] = sapply(1:nrow(index.delta.1), function(x) {
      truncnorm::rtruncnorm(1, a = -Inf, b = 0, mean = kappa0[index.delta.1[x,2]], sd = 1)
    })
    Z[index.delta.0] = sapply(1:nrow(index.delta.0), function(x) {
      truncnorm::rtruncnorm(1, a = 0, b = Inf, mean = kappa0[index.delta.0[x,2]], sd = 1)
    })
    
    
    ### update kappa0 
    pos.v.k = 1/(n + 1/sig_kappa)
    kappa0 = sapply(1:J, function(x) rnorm(1) * sqrt(pos.v.k) + pos.v.k * (mu_kappa/sig_kappa + sum(Z[,x])) )
    ...
    if(i > floor(niter/2)){
      ls[[i-floor(niter/2)]] = list(Lambda = Lambda, sig2 = sig2, vs2 = vs2, phi = phi, tau.k = tau.k, eta = eta, ri = ri, thetaj = thetaj, sij = sij, smj = smj, delta = delta, eps.j = eps.j, kappa0 = kappa0)
    }
  }
  return(ls)
}
