...
ZI_MLN_wihout <- function(Y, M = M, m = m, K=10, niter = 20000, seed = 3, a.phi = 1/2, a.tau = 1, b.tau = 1/50, a.sig = 3, b.sig = 3,
  ...
  for(i in 1:niter){
    ...
    Z[index.delta.0] = sapply(1:nrow(index.delta.0), function(x) {
      truncnorm::rtruncnorm(1, a = 0, b = Inf, mean = kappa0[index.delta.1[x,2]], sd = 1)
    })
    ...
    if(i > floor(niter/2)){
      ls[[i-floor(niter/2)]] = list(Lambda = Lambda, sig2 = sig2, vs2 = vs2, phi = phi, tau.k = tau.k, eta = eta, ri = ri, thetaj = thetaj, sij = sij, smj = smj, delta = delta, eps.j = eps.j, kappa = kappa)
    }
  }
  return(ls)
}
