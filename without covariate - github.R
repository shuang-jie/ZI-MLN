...
sum(apply(Lambda.true, 1, function(x) all(x==0))) / J
...
max(true.cor[-which(true.cor==1)])
min(true.cor[-which(true.cor==1)])
corrplot::corrplot(true.cor, tl.pos = 'n')
sum(Omega.true[upper.tri(Omega.true, diag = T)]==0) / 10325
...
vr.theta = mean(hat.thetaj); Lr.theta = 15; a.phi.theta = 1; a.omega.theta = b.omega.theta = 5; #ur2.theta = var(hat.thetaj)
ur2.theta = 1
a.xi.theta = vr.theta
sigma2.xi.theta = 1
...
for(i in 1:niter){
  ...
  for(isample in 1:n){
    for(j in 1:J){
      if( Y[isample, j] >0){
        ...
        prob.mean = ri[isample] + thetaj[j] + sum(Lambda[j,] * eta[isample,]) + sij.matrix[isample, j]
  ...
  for(j in 1:J){
    ...
    Dj.inv = diag(1 / (psi.matrix[j, ] * phi.vector[j]^2 * tau.k^2))
    if(inherits(try(chol2inv(chol((Dj.inv+1/sigma2 * t(Lambda[index.lambda,, drop=F]) %*% Lambda[index.lambda,, drop=F]))), silent = T), "try-error")){
      Sigmaj = Dj - Dj %*% solve(Dj + sigma2 * solve( t(eta[index.lambda,, drop=F]) %*% eta[index.lambda,, drop=F])) %*% Dj
    } else {
      # Sigmaj = solve(Dj.inv+1/sigma2 * t(eta[index.lambda,, drop=F]) %*% eta[index.lambda,, drop=F])
      Sigmaj = chol2inv(chol(Dj.inv+1/sigma2 * t(eta[index.lambda,, drop=F]) %*% eta[index.lambda,, drop=F]))
    }
  ...
  ### Sigma2
  inter.RSS = y.star - sij.matrix - matrix(rep(ri,J), ncol = J) - 
